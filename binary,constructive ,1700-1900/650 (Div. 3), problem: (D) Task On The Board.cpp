...
void solve(){ll n ;string s,p; ci s;p=s;
    ci n ; 
 int a[n];inarr(a,0,n);
vector<vector<int>> z0;
map<char,int> dp ;
rep(i,0,sz(s)){dp[s[i]]++;}
while(1){vector<int> cur;
    rep(i,0,n)if(a[i]==0){cur.pb(i);a[i]=oo;}
    if(sz(cur)==0)break;
    z0.pb(cur);
    rep(i,0,n)for(auto x:cur){a[i]-=abs(x-i);}
}
auto it=dp.rbegin();
p=string(n,'.');
for(auto cur:z0){
    while(it->ss < sz(cur))it++;
    for(auto f:cur)p[f]=it->ff;
     it++;   
}
