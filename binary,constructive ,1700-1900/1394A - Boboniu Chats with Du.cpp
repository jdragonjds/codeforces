...
inline void yn(int x) { if(!x)cout << "NO\n";  else if(x)cout << "YES\n";return; }
  ...
  const int N = 2e6+7 ,M=1e6 ,mod=1e9+7;
  const int MAXA = 0; 
 //int mind[MAXA+5]={0};
...
while(tc--) { int d,m,fun=0 ,t;
cin>>n>>d>>m;
vecll g ,s ;  

rep(i,0,n){ cin>> t;
if(t>m)g.pb(t);else s.pb(t);}

sort(rall(g));sort(rall(s));

rep(i,1,sz(s))s[i]+=s[i-1];
rep(i,1,sz(g))g[i]+=g[i-1];

if(sz(g)==0){co s[n-1];continue;}
//co sz(s)<<" "<<sz(g)<<endl;
fun=0; 
