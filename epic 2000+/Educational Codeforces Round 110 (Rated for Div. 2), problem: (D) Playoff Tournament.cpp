...
void solve(){ci k;string s; ci s; n=sz(s);
reverse(all(s));
int q;ci q;
while(q--){
 int t; ci t;t=n-t;ci s[t];
 int i=0,l=1,r=2;t=0;

 queue<int> q;
 if(s[0]=='?') q.pb(l),q.pb(r);
 else if(s[i]=='1')q.pb(l);
 else q.pb(r);

    while(sz(q)){
        i=q.front();q.pop();l=2*i+1;r=2*i+2;       
         //co s[i]<<" "<<i<<endl;
        if(l>=sz(s)){ if(s[i]=='?')t+=2;else t++; continue;}

      if(s[i]=='?') q.pb(l),q.pb(r);
      else if(s[i]=='1')q.pb(l);
      else q.pb(r);
