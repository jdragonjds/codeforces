...
int n,i=2;
    ...
    for(int i=0;i<n-1;++i){
        ...
        v[y].push_back(x);
        int t=__gcd(a[x],a[y]);
        if(t>1)dp.pb({t,x,-1,-1,y});
    }
if(sz(dp)==0 && a[0]==1){co 0<<endl;return;}
if(n==1 || sz(dp)==0){co 1<<endl;return;}
 
while(sz(dp)){d.clear();
  rep(i,0,sz(dp)){
   for(auto x:v[dp[i][1]]){
    if(x==dp[i][2] || x==dp[i][4])continue;
     int t=__gcd(dp[i][0],a[x]);
     if(t>1)d.pb({t,x,dp[i][1],dp[i][3],dp[i][4]});
      }
   for(auto x:v[dp[i][4]]){
    if(x==dp[i][3] || x==dp[i][1])continue;
        int t=__gcd(dp[i][0],a[x]);
        if(t>1)d.pb({t,dp[i][1],dp[i][2],dp[i][4],x});
      }
  }

/*
rep(i,0,sz(d)){
co d[i][0]<<" "<<d[i][1]<<" "<<d[i][2]<<" "<<d[i][3]<<" "<<d[i][4]<<" "<<endl;
